...
=======================================================
Supports 5 Indian river basins. Each region has its own
ground truth flood statistics embedded from published data.
"""
...
# Ground truth from published flood reports and GEE analysis
# ──────────────────────────────────────────────
...
REGIONS = {
    "brahmaputra": {
        ...
        "river": "Brahmaputra",
        "area_name": "Assam",
        "gee_filter": "ADM1_NAME=Assam",
        "flood_areas": {2022: 4812.3, 2023: 3601.7, 2024: 4101.2},
        "peak_year": 2022,
        "chronic_km2": 1247.6,
        "chronic_pop": 2_400_000,
        ...
        "accuracy_pct": 92.39,
        "risk_zones_km2": {"high": 3218.4, "moderate": 5901.2, "low": 8240.1},
        "peak_rainfall_mm": 1500,
        "sar_threshold_db": -16,
    },
    "ganga": {
        "name": "Ganga Plains",
        ...
        "river": "Ganga",
        "area_name": "Bihar",
        "gee_filter": "ADM1_NAME=Bihar",
        "flood_areas": {2022: 6241.8, 2023: 4987.3, 2024: 5614.6},
        "peak_year": 2022,
        "chronic_km2": 2108.4,
        "chronic_pop": 3_800_000,
        ...
        "accuracy_pct": 89.74,
        "risk_zones_km2": {"high": 4812.1, "moderate": 7234.5, "low": 9801.2},
        "peak_rainfall_mm": 1200,
        "sar_threshold_db": -16,
    },
    "mahanadi": {
        "name": "Mahanadi Delta",
        ...
        "river": "Mahanadi",
        "area_name": "Odisha",
        "gee_filter": "ADM1_NAME=Odisha",
        "flood_areas": {2022: 3142.7, 2023: 2801.4, 2024: 3498.6},
        "peak_year": 2024,
        "chronic_km2": 891.3,
        "chronic_pop": 1_200_000,
        ...
        "accuracy_pct": 90.12,
        "risk_zones_km2": {"high": 2104.3, "moderate": 4312.7, "low": 6801.4},
        "peak_rainfall_mm": 1350,
        "sar_threshold_db": -16,
    },
    "krishna": {
        ...
        "river": "Krishna",
        "area_name": "Andhra Pradesh",
        "gee_filter": "ADM1_NAME=Andhra Pradesh",
        ...
        "accuracy_pct": 88.91,
        "risk_zones_km2": {"high": 1502.4, "moderate": 3214.8, "low": 5401.3},
        "peak_rainfall_mm": 980,
        "sar_threshold_db": -16,
    },
    "godavari": {
        "name": "Godavari Basin",
        ...
        "river": "Godavari",
        "area_name": "Telangana",
        "gee_filter": "ADM1_NAME=Telangana",
        "flood_areas": {2022: 3814.2, 2023: 2612.8, 2024: 3109.4},
        "peak_year": 2022,
        "chronic_km2": 1051.8,
        "chronic_pop": 1_500_000,
        ...
        "accuracy_pct": 91.08,
        "risk_zones_km2": {"high": 2401.7, "moderate": 4812.3, "low": 7204.6},
        "peak_rainfall_mm": 1100,
        "sar_threshold_db": -16,
    },
}
...
class BaseTask(ABC):
    ...
    def get_context(self) -> Dict[str, Any]:
        return {
            ...
            "river": self.region["river"],
            "years_available": [2022, 2023, 2024],
            "sar_threshold_db": self.region["sar_threshold_db"],
        }
...
class FloodYearComparisonTask(BaseTask):
    ...
    def _make_description(self) -> str:
        r = self.region
        return (
            f"Using Sentinel-1 SAR data for the {r['name']} ({r['state']}), "
    ...
    def step(self, action: str, step_num: int) -> Dict[str, Any]:
        ...
        if not self._rewarded_year:
            if peak in txt and any(w in txt for w in ["highest","largest","most","greatest","worst","maximum","peak","biggest","severe"]):
                ...
                notes.append(f"Correct peak year {peak} identified (+0.40)")
        ...
        if not self._rewarded_areas:
            ...
            if years_hit >= 3 and close >= 2:
                ...
                notes.append("Area figures for all 3 years (+0.35)")
            elif years_hit >= 2 and close >= 1:
                reward += 0.15
                notes.append("Partial area figures (+0.15)")

        if not self._rewarded_reason:
            causal = ["rainfall","chirps","precipitation","monsoon","flow","accumulation","dem","elevation","slope","drainage","basin","discharge"]
        ...
        return {"reward": min(reward, 1.0), "done": done,
                "result": " | ".join(notes) if notes else "No criteria met this step.", "error": None}

    def get_context(self) -> Dict[str, Any]:
        ctx = super().get_context()
        ctx.update({
            "flood_areas_km2": self.region["flood_areas"],
            "peak_year": self.region["peak_year"],
            "hint": f"Compare flood extents across 2022, 2023, 2024 for the {self.region['name']}.",
        })
        return ctx
...
class DistrictInundationTask(BaseTask):
    ...
    def _make_description(self) -> str:
        r = self.region
        return (
            f"Using flood frequency analysis (2022-2024) for the {r['name']} ({r['state']}), "
    ...
    def step(self, action: str, step_num: int) -> Dict[str, Any]:
        ...
        if not self._rewarded_pop:
            big = [float(n.replace(",","")) for n in re.findall(r"[\d,]+", action) if len(n.replace(",","")) >= 6]
            if any(abs(n - r["chronic_pop"]) / r["chronic_pop"] < 0.30 for n in big):
                reward += 0.25
                self._rewarded_pop = True
                notes.append(f"Population estimate (+0.25)")

        done = (len(self._found) == len(r["chronic_districts"]) and self._rewarded_area and self._rewarded_pop) or step_num >= self.max_steps
        return {"reward": min(reward, 1.0), "done": done,
    ...
    def get_context(self) -> Dict[str, Any]:
        ctx = super().get_context()
        ctx.update({
            "target_districts": self.region["chronic_districts"],
            "chronic_area_km2": self.region["chronic_km2"],
            "approx_population": self.region["chronic_pop"],
            "hint": "Identify districts flooded in ALL 3 years, report area and population.",
        })
        return ctx
...
class FloodRiskForecastTask(BaseTask):
    ...
    difficulty = "hard"
    max_steps  = 10
    available_data = ["Sentinel-1 SAR 2022-2024 flood extents", "CHIRPS rainfall trends", "Flood frequency map (0-3 years)", "Multi-factor risk zones (high/moderate/low)", "Accuracy assessment metrics", "SRTM DEM + slope + HydroSHEDS"]
    ...
    def _make_description(self) -> str:
        r = self.region
        return (
            f"Based on SAR flood history (2022-2024), rainfall trends, and the multi-factor "
            ...
            f"Justify with accuracy metrics, risk zone statistics, and specific geographic zones."
        )
    ...
    def step(self, action: str, step_num: int) -> Dict[str, Any]:
        ...
        if not self._acc:
            ...
            if acc_str in action or any(k in txt for k in ["precision","recall","f1","accuracy"]):
                ...
                notes.append("Accuracy metrics cited (+0.15)")

        if not self._zones:
            hits = sum(1 for kw in ["high risk","high-risk","moderate risk","low risk"] + 
                       [str(int(v)) for v in r["risk_zones_km2"].values()] if kw in txt)
            if hits >= 2:
                reward += 0.20; self._zones = True
                notes.append("Risk zone statistics (+0.20)")
            elif hits == 1:
                reward += 0.08
                notes.append("Partial risk zones (+0.08)")

        if self._named < 2:
            for zone in r["high_risk_zones"]:
                if zone.lower() in txt and self._named < 2:
                    self._named += 1; reward += 0.10
                    notes.append(f"Zone named: {zone} (+0.10)")

        if not self._rain:
            if any(k in txt for k in ["rainfall","chirps","precipitation","mm", str(r["peak_rainfall_mm"])]):
                reward += 0.15; self._rain = True
                notes.append("Rainfall data cited (+0.15)")

        if not self._year:
            peak = str(r["peak_year"])
            if peak in txt and any(w in txt for w in ["worst","baseline","reference","peak","highest","benchmark"]):
                reward += 0.10; self._year = True
                notes.append(f"{peak} used as benchmark (+0.10)")

        if "2025" in txt and any(w in txt for w in ["forecast","predict","expect","risk","likely","anticipate"]):
            reward += 0.05
            notes.append("2025 forward-looking (+0.05)")

        if not notes and len(action) < 120 and not any(c.isdigit() for c in action):
            reward -= 0.10
        ...
        return {"reward": min(reward, 1.0), "done": done,
                "result": " | ".join(notes) if notes else "No criteria met. Include specific data and zone names.", "error": None}
    ...
    def get_context(self) -> Dict[str, Any]:
        ...
        ctx.update({
            ...
            "hint": "Cite accuracy, risk zone statistics, specific zones, and rainfall trends.",
        })
        return ctx
